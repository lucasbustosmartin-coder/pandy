...
5. **Replicar matriz:** copia **todas** las filas de un par origen `(código, usa_intermediario)` a un **código destino** y flag intermediario destino. **No corre** si el destino ya tiene al menos una fila (hay que borrar o editar a mano antes). Si no existe el tipo en catálogo, pide confirmación explícita antes de insertar. **No sustituye monedas** (ej. si copiás desde USD-ARS, las celdas `moneda` que eran ARS siguen siendo ARS: para USD-EUR hay que cambiar ARS→EUR a mano o usar **`sql/migracion_reglas_eur_usd_desde_usd_ars_ars_usd_int.sql`**). Ver **`docs/REGLAS_CRUCE_INVERSO_CONSISTENCIA.md`**.
