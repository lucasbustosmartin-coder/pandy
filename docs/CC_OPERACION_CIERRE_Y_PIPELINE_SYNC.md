...
Inventario explícito de **dónde** el sistema puede fallar ese criterio (tres vías de sync, exclusiones del motor, MC manual, RPC sin validación, etc.): **`docs/CC_GRIETAS_INVARIANTE_SALDO_CERO_ORDEN.md`** (tres vías de sync y riesgos). **Neteo duro antes de persistir:** `sincronizarCcYCajaDesdeOrden` valida con `validarInvarianteNeteoCcClienteAcuerdoCerrado` y **no** invoca `sync_cc_caja_orden` si la CC del cliente del acuerdo no netea por moneda (motor, legacy y MC manual), **solo** cuando ya no hay transacciones en **pendiente** y se cumple uno de: MC completo, par clásico sin MC, u orden en estado de cierre instrumentación — ver `CC_GRIETAS_INVARIANTE_SALDO_CERO_ORDEN.md` (mitigaciones). El residual permitido incluye leyendas **Pandy** o **Tercero** «cumple pata», la pata en moneda entregada en cruces sin int. (doble pata), y la fila sintética «Comisión del acuerdo» sin `transaccion_id` — ver `INSTRUMENTACION_MANUAL_MULTICONTRAPARTE.md` §3 y `CC_GRIETAS_INVARIANTE_SALDO_CERO_ORDEN.md` §10.
