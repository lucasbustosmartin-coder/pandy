...
- **Patrón cp_ic (Cliente→Pandy + Intermediario→Cliente):** además de la comisión explícita, en **CC intermediario** entran movimientos por el egreso **Intermediario→Cliente** (`entidad_cc = intermediario`, `monto_transaccion` con signo negativo al cerrar el par), de modo que el saldo refleje **me + comisión del intermediario** (lo que Pandy le debe al int. tras cobrar mr al cliente). Sin esas filas solo quedaría la comisión y no el pago del int. al cliente.
