...
| Dónde | Acción | Cumple regla |
|-------|--------|---------------|
| Flujo unificado (`solicitarConfirmacionYAnularOrden` / `ejecutarAnulacionOrdenCompleta`) | Siempre: `ordenes.estado = 'anulada'`; **DELETE** de `comisiones_orden` y `orden_comisiones_generadas` para ese `orden_id` (evita comisiones «colgadas» en pendiente o datos huérfanos); transacciones de la instrumentación → `anulada` (también si la orden estaba en `orden_ejecutada`, con confirmación reforzada en UI). **Siempre después:** `sincronizarCcYCajaDesdeOrden(ordenId)` — borra y vuelve a insertar la CC/caja **derivada** de esa orden: filas CC ligadas a transacciones anuladas quedan con `estado` **anulado** (visibles en la vista, no suman al saldo); la caja solo refleja transacciones que hubieran estado **ejecutadas**. Si la orden ya está `anulada` pero en BD alguna transacción sigue distinta de `anulada` (p. ej. `pendiente` o `ejecutada`, datos viejos), al entrar al sync se intenta **persistir** coherencia: `UPDATE transacciones` de esa instrumentación a `anulada` y `fecha_ejecucion = null` antes de calcular CC; si el UPDATE falla (p. ej. RLS), se alinean estados **en memoria** para el cálculo y se registra advertencia en consola. Además, si hay comisión sintética sin `transaccion_id`, el sync **fuerza** `estado = anulado` en toda la CC derivada no manual antes de persistir, para que un resync no vuelva a dejar **pendiente**. Si el sync falla, la anulación de orden rechaza con error (`propagarError` en el sync). La función `anularMovimientosCcYCajaNoManualPorOrden` quedó como utilidad legada, no en este flujo. | Sí. |
