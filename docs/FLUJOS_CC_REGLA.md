...
| Dónde | Acción | Cumple regla |
|-------|--------|---------------|
| Flujo unificado (`solicitarConfirmacionYAnularOrden` / `ejecutarAnulacionOrdenCompleta`) | **Hacia adelante (órdenes que se anulan después de este flujo):** cartel de impacto (saldos actual → después por entidad/moneda) y confirmación. Luego: `ordenes.estado = 'anulada'`; **DELETE** de `comisiones_orden` y `orden_comisiones_generadas`; transacciones de la instrumentación → `anulada`; `sincronizarCcYCajaDesdeOrden`; **red de seguridad** `anularMovimientosCcYCajaNoManualPorOrden` (CC derivada cliente e intermediario y caja de **esa** orden a `anulado`; no toca CC manual ni órdenes ya anuladas de historial). **Verificación:** no debe quedar transacción ≠ anulada ni CC/caja derivada viva; si queda, error (la orden puede haber quedado anulada) y auditoría igual. Si el sync falla, igual se aplica la red de seguridad. El Refrescar global **sigue omitiendo** órdenes anuladas (no reescribe historial). | Sí. |
| Confirmación y auditoría | Cartel HTML con impacto en CC (y caja si hay). `auditoria_app` categoría `orden` acción `anular`: **quién** (`usuario_id` + `usuario_snapshot`); **detalle** legible; **`metadata.cambios`** una fila por registro (orden, cada transacción, cada CC/caja derivada viva, comisiones eliminadas); **`metadata.registros`** con ids/montos/estado anterior. Permiso `anular_orden`. RLS: `sql/migracion_rls_anular_orden_cc_caja.sql`. | — |
...
- **Crear Ganancia / Comisión**: se usan `asegurarGananciaPandy` y `asegurarComisionIntermediario`, que consultan la tabla y solo crean movimientos si aún no existe la fila. **Comisión intermediario:** por defecto un egreso en `movimientos_caja` efectivo (legacy); en **CHEQUE-ARS** (matriz) el egreso en caja va siempre como **transferencia bancaria** (`caja_tipo: banco`). Si aplica tasa transferencia fuera del acuerdo (`ordenOmitirMovimientoCajaComisionIntPorTasaTransferenciaFueraAcuerdo` en `main.js`), solo se inserta la fila en `orden_comisiones_generadas` sin `movimientos_caja`. La línea «Comisión del acuerdo» en CC intermediario la inserta el flujo previo (`insertarFilasComisionIntermediarioCcPorTransaccion`).
