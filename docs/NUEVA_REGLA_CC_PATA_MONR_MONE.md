...
Implementación: comisión en `aplicarMotorCcDesdeReglasDeNegocio` (USD-USD / cruces TC); MonE en intermediario en multicontraparte manual `aplicarCcMulticontraparteManualTrx` (pagador intermediario, `pago_realizado`). Fuera de rollout **y** sin wizard de **tasa por transferencia al intermediario** activo, la comisión sintética sigue en **positivo** por magnitud (`Math.abs` frente a `signo` en matriz) donde ya aplicaba. Con `intermediario_pago_transferencia` + `intermediario_transferencia_cobra_tasa` y tasa estrictamente positiva, la comisión del intermediario en ese motor va en **−** aunque el patrón amplio MonR/MonE no califique aún (p. ej. instrumentación P→I + I→C).
