...
- **P,P (par cliente todo pendiente):** `todoParClientePendienteConIntParaAlinearCcMotor` + lógica en `aplicarMotorCcDesdeReglasDeNegocio` evita duplicar **mr/me** entre ingreso `compromiso_cobrar` y egreso pendiente **+mr/−me** en CC cliente: el ingreso pasa a signo de **cobro_realizado** (−mr/−me o −`monto_transacción` según fila) y el egreso de entrega queda en **una** línea **compromiso_pago** por el monto de la transacción (misma idea que USD-USD+int).
