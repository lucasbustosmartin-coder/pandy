...
- **Patrón `cp_ic`** (Cliente→Pandy + Intermediario→Cliente) para **USD-ARS+int:** filas en `sql/reglas_de_negocio_tabla.sql` (bloque USD-ARS true `cp_ic`); carga puntual análoga a `sql/insert_reglas_ars_usd_int_cp_ic_si_faltan.sql` si hiciera falta. **E,E `cp_ic`:** si con ambas ejecutadas la CC cliente no cierra, aplicar `sql/migracion_reglas_cp_ic_ee_neteo_cliente_cruzadas.sql` (líneas 1–2 en egreso con `contrapartida_ejecutada = true`).
