...
**`comisión = importe − monto_entregado`** (equivale a que la tasa actúe como recargo *inclusivo* sobre lo entregado, no como descuento lineal sobre el importe recibido).

Ejemplo: importe 5.300 USD y tasa 6 % → entrega 5.300 / 1,06 = **5.000 USD**, comisión **300 USD**.
...
- **Única fuente de verdad:** tabla **`reglas_de_negocio`** (`usa_intermediario = false`), modelo simple: solo movimientos CC coherentes con orden/transacciones (sin espejo ni flags “suma saldo”).
  ...
  - **E,P** (ingreso ejecutado, egreso pendiente): en la **misma** transacción de egreso pendiente (`contrapartida_ejecutada = true`) van **dos filas** (`linea` 0 y 1): **`+mr`** y, en la tabla, **`−me`** en `linea` 1. **Sin** línea explícita de comisión en el catálogo, el saldo neto queda **−me**. **Con** la fila **`es_comision`** `pendiente`/`false` (+ **`mr_menos_me`**), sumar además **−me** duplicaría el spread; el motor (`main.js`) aplica en ese caso la segunda línea como **−mr** (compromiso de entrega nominal), y la comisión **+ (mr − me)** deja el neto en **−me** (p. ej. −5.000 con acuerdo 5.300 / 5.000 y comisión 300). Visible en Movimientos; **G/P** sigue sin contar pendientes.
...
- Montos fijos de ejemplo: `tests/e2e/cc-tipos-activos-esperado.js` → `USD_USD_FIJOS` y `COMBINACIONES_USD_USD` (5.300 / 5.000 / comisión 300 con tasa cliente 6 %).
