...
**Órdenes — par vinculado prohibido en la misma fila (Fase 4):** si `ordenes.cliente_id` y `ordenes.intermediario_id` coinciden con un par en `contraparte_vinculo`, el **INSERT/UPDATE** de la orden se rechaza (trigger en BD + validación en `main.js`). En bases ya desplegadas ejecutar **`sql/migracion_ordenes_validar_no_par_vinculado_fase4.sql`** (idempotente). Incluida en el bootstrap dev tras la migración de `contraparte_vinculo`.

**Registro Pandi:** el script **`migracion_ordenes_validar_no_par_vinculado_fase4.sql`** fue **ejecutado en Supabase desarrollo** (proyecto enlazado a Vercel Preview / preview.pandi.company) **y en Supabase producción** (pandi.company). Bitácora Log (Excel) y este párrafo quedan como constancia operativa.
