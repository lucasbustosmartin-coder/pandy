...
En **cp_ic** con **E,E**, el libro **Pandy–cliente** debe **netear a cero** en USD y en ARS cuando ambas patas están ejecutadas: el cobro en **monR** se compensa con el cierre sintético **+monR** «Cierre orden»; la entrega en **monE** vía **Intermediario→Cliente** **no** se imputa como **−monE** en CC cliente (evita “deuda ARS” del cliente frente a Pandy por pesos que liquidó el intermediario). La posición en **monE** del circuito con el intermediario queda en **CC intermediario**. El ingreso Cliente→Pandy puede llevar **par ±** en monR si hace falta (reglas / migración `sql/migracion_reglas_cp_ic_ingreso_ee_par_moneda_recibida.sql`). Ver `docs/REG_NEG_ARS_USD_INT_PASO1.md` / `REG_NEG_USD_ARS_INT_PASO1.md`.
