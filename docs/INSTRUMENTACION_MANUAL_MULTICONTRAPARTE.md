...
- **Terceros e intermediario en patas:** mismos criterios de signo **pagador − / cobrador +** donde aplique; patas con intermediario delegan en `aplicarCcMulticontraparteManualTrx`. **Ingreso ejecutado** con pagador = cliente del acuerdo y cobrador = intermediario lleva en la CC del acuerdo el mismo **neteo −m (cobro) +m (ajuste libro)** que el pago a otro cliente, más la línea en CC del intermediario.
