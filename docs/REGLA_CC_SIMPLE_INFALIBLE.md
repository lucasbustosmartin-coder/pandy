...
- **Cierre orden en dos monedas (ARS-USD, USD-ARS):** Dos movimientos de cierre en CC cliente **solo cuando ambas** patas del acuerdo con el cliente están ejecutadas: cobro al circuito (ingreso Cliente→Pandy o Cliente→Intermediario) y entrega al cliente (egreso Pandy→Cliente **o** Intermediario→Cliente, p. ej. patrón **cp_ic**). Si solo una está ejecutada, no hay cierre y el saldo muestra solo la moneda de la transacción ejecutada.
- **Con intermediario y motor `reglas_de_negocio`:** `sincronizarCcYCajaDesdeOrden` aplica el mismo **cierre sintético** (+suma ingresos cliente en monR, −suma egresos hacia cliente en monE) cuando ambos totales son > 0, alineado con `ingresoDesdeClienteHaciaPandyOIntermediarioEjecutado` y `egresoEntregaAClienteEjecutado`. La condición “con intermediario” coincide con el fetch de reglas (`usa_intermediario` en catálogo o `intermediario_id` en la orden). Pagador/cobrador efectivos y estado Ejecutada como en el sync. No aplica si falta ingreso desde el cliente hacia Pandy/intermediario ejecutado o falta egreso de entrega al cliente ejecutado.
