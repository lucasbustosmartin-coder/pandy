...
- **Cierre orden en dos monedas (ARS-USD, USD-ARS):** Dos movimientos de cierre en CC cliente cuando **no** corre el motor **completo** (`usarMotorEfectivo`), con **ambas** patas ejecutadas: cobro al circuito (ingreso Cliente→Pandy o Cliente→Intermediario) y entrega al cliente (egreso Pandy→Cliente **o** Intermediario→Cliente). Eso incluye órdenes con **`instrumentacion_ajustada_manual`** o multicontraparte + reglas: el motor va en `soloComisiones` y **no** recorre transacciones, y el bucle legacy no inserta egreso Intermediario→Cliente en CC cliente (`pag === 'intermediario'`); el cierre sintético **+monR en monR y −monE en monE** evita quedar solo con el cobro en una moneda sin la contrapartida libro. Si solo una pata está ejecutada, no hay cierre y el saldo muestra solo la moneda de la transacción ejecutada.
