...
**UI (resumen Saldos, Excel de resumen y cards del modal detalle):** el importe visible sigue **óptica Pandy** pero el signo depende de **qué tipo de pendiente** hay en esa moneda (según `transacciones.tipo`, `pagador`, `cobrador` de los movimientos en estado pendiente): si solo es **cobro a Pandy** (ingreso con cobrador pandy) se **invierte** el signo respecto de la suma algebraica (−a), con leyenda «Pendiente de cobro»; si solo **Pandy debe pagar** (egreso con pagador pandy) se muestra el **mismo signo** que el algebraico, con «Pendiente de pago»; si hay **mezcla** de ambos o no aplica la regla, se usa el fallback **−a** como antes. El detalle de líneas y los tests de expectativa numérica siguen usando la convención algebraica en base; los E2E comparan con `min(|leído + E|, |leído − E|) ≤ 1` frente al esperado algebraico **E** (`tests/e2e/cc-resumen-optica-match.js`).
