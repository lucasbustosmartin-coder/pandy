...
- **Reload tras limpiar base:** `tests/e2e/e2e-reload-app.js` (`reloadYEsperarAppLista`) — espera hasta ~75 s a que el sidebar pase a visible tras `getSession`/permisos; si aparece login (sesión expirada), vuelve a autenticar. Lo usan **02** y **03** en lugar de un único `toBeVisible` de 15 s (en corridas largas o red lenta el sidebar sigue en `display:none` y Playwright lo marca *hidden*).
