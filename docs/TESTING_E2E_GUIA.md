...
**Regresión comisión CC (sync global):** **`tests/e2e/92-cheque-ars-comision-acuerdo-invariante.spec.js`** — una orden CHEQUE-ARS 4× ejecutada; comprueba que en Movimientos exista **«Comisión del acuerdo»** con monto **≈ mr−me** y que **siga** tras **`#cc-btn-refrescar`** (sync de todas las órdenes). Comando: **`npm run test:e2e-cc-cheque-comision-invariante`**. Cliente dedicado **`E2E CHEQUE Comisión Invariante`** (no coincide con `E2E %` de la RPC; el spec llama a `limpiarBaseE2eDesdeTests` al inicio). Mismos prerrequisitos Supabase que **91** / **01** (`reglas_de_negocio`, RPC `sync_cc_caja_orden`).
...
Cubre las **4 combinaciones** de estados **Tx1/Tx2** (`P,P`, `E,P`, `P,E`, `E,E`) para **ARS-USD**, **USD-ARS**, **USD-USD** (sin intermediario) y **USD-USD con intermediario** (mismas 4 combinaciones; expectativas cliente/caja como sin int; CC intermediario USD solo en **E,E**, mitad de la comisión con split 50/50), con **montos enteros fijos** en `tests/e2e/cc-tipos-activos-esperado.js` (`COMBINACIONES_USD_USD_INT`). Misma dinámica: limpieza E2E al inicio de cada caso, cliente `E2E CC TiposActivos`, intermediario `E2E CC TiposActivos Int` solo en el bloque USD-USD+int, validación de saldos USD/ARS en resumen CC, detalle (modal **Ver detalle**), caja **Banco USD + Banco ARS** (la plantilla 2 tx sin CHEQUE-ARS usa **transferencia** por defecto → el sync no mueve **efectivo** en esas monedas; la semilla RPC carga el mismo nominal en efectivo y banco, así que el assert compara contra **banco**), **Efectivo EUR** solo cuando el tipo cruza EUR (EUR-USD, USD-EUR, EUR-ARS, ARS-EUR; en USD-ARS / ARS-USD / USD-USD el test **no** exige coincidencia en EUR para no depender de ruido ajeno a la orden), y columnas **Exp/Real/Rdo Int USD** en el log Excel cuando aplica.
...
6. **`92-cheque-ars-comision-acuerdo-invariante.spec.js`** — CHEQUE-ARS: comisión del acuerdo visible y estable tras Refrescar CC (`npm run test:e2e-cc-cheque-comision-invariante`).

En terminal verás líneas `[E2E 1/6]` … `[E2E 6/6]` y subpasos por combinación. **Solo matriz de activos (sin 91 / sin repetir tipos):** `npm run test:e2e-cc-activos-completo` (01+02+03). La suite completa `npm run test:e2e` incluye además 90-login, 91 (smoke + reversa) y 92 (comisión CHEQUE-ARS + Refrescar CC).
...
| `tests/e2e/92-cheque-ars-comision-acuerdo-invariante.spec.js` | CHEQUE-ARS 4× ejecutada: saldo cliente ARS ≈ 0, fila **Comisión del acuerdo** (≈ mr−me) en Movimientos y persiste tras **Refrescar** CC global. `npm run test:e2e-cc-cheque-comision-invariante`. |
