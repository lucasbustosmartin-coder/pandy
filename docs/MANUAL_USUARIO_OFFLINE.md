...
- **Control de calidad** (menú lateral, permiso **ver_control_calidad**; quien tenía el permiso heredado de control G/P puede seguir entrando): informe por período (día / semana / mes / total) con alertas de parejas **cuenta corriente ↔ caja** y chequeos de transacciones del **cliente** frente a movimientos en **cuenta corriente** (incluye ejecutadas sin filas CC, ejecutadas con estados CC distintos de cerrado, pendientes con estados CC distintos de pendiente). Sin conexión no se puede cargar.  
